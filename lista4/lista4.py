...
try:
    ...
    print("2) O resultado da derivada da função para x = {} é zero, "
          "portanto ocorre uma divisão por zero durante a resolução pelo método de Newton.".format(str(ex)))
except NaoConverge:
...
J = lambda x: [[64 * x[0]**3, 64 * x[1]**3, 4 * x[2]**3],
               ...
               [3 * x[0]*x[0], -1, 1]]
try:
    print("1) Newton N equações:", SolucaoEqNLinearNewton([0.5, 1, 2], 0.0001, 1000, J, F))
except NaoConverge:
    print("1) Método por Newton para N equações não converge")

J = 3 #jacobiana de partida ??
#try:
#    print("2) Método de Broyden:", SolucaoEqNLinearBroyden([0.5, 1, 2], J, 0.0001, 1000, F))
#except NaoConverge:
#    print("2) Método de Broyden não converge")
...
theta1, theta2 = 0, 3
try:
    print("Newton:", SolucaoEqNLinearNewton([0, 1, 2], 0.00001, 1000, J, F))
except NaoConverge:
    print("Método de Newton não converge")
